...
+ (LHSecIdentityRef)identityWithFile:(NSString *)filePath password:(NSString *)password error:(NSError *__autoreleasing *)error
{
  ...
  if (!pkcs12Data.length) {
    [self errorWithMessage:@"ErrorPKCS12EmptyData" error:error];
    return NULL;
  }
  return [self identityWithPKCS12Data: pkcs12Data password:password error:error];
}
...
+ (LHSecIdentityRef)identityWithPKCS12Data:(NSData *)pkcs12 password:(NSString *)password error:(NSError *__autoreleasing *)error
{
  ...
  if (identities.count == 0) {
    [self errorWithMessage:@"ErrorPKCS12NoItems" error:error];
    return NULL;
  }
  if (identities.count > 1) {
    [self errorWithMessage:@"ErrorPKCS12MultipleItems" error:error];
    return NULL;
  }
...
+ (NSArray *)identitiesWithPKCS12Data:(NSData *)pkcs12 password:(NSString *)password error:(NSError *__autoreleasing *)error
{
  ...
  for (NSDictionary *dict in dicts) {
    LHSecIdentityRef identity = dict[(__bridge id)kSecImportItemIdentity];
    if (identity) {
      LHCertificateRef certificate = [self certificateWithIdentity:identity error:error];
      if (!certificate) {
        return nil;
      }
      if ([self isPushCertificate:(__bridge SecCertificateRef)(certificate)]) {
        LHSecKeyRef key = [self keyWithIdentity:identity error:error];
        if (!key) {
          return nil;
        }
...
+ (NSArray *)allIdentitiesWithPKCS12Data:(NSData *)data password:(NSString *)password error:(NSError *__autoreleasing *)error
{
  ...
  if (status != errSecSuccess || !items) {
    switch (status) {
      case errSecDecode:
        [self errorWithMessage:@"ErrorPKCS12Decode" error:error];
        return [NSArray array];
        
      case errSecAuthFailed:
        [self errorWithMessage:@"ErrorPKCS12AuthFailed" error:error];
        return [NSArray array];
#if !TARGET_OS_IPHONE
      case errSecPkcs12VerifyFailure:
        [self errorWithMessage:@"ErrorPKCS12Password" error:error];
        return [NSArray array];
        
      case errSecPassphraseRequired:
        [self errorWithMessage:@"ErrorPKCS12PasswordRequired" error:error];
        return [NSArray array];
#endif
    }
    [self errorWithMessage:@"ErrorPKCS12Import" error:error];
    return [NSArray array];
  }
  return dicts;
}
...
+ (LHCertificateRef)certificateWithIdentity:(LHSecIdentityRef)identity error:(NSError *__autoreleasing *)error
{
  ...
  LHCertificateRef certificate = CFBridgingRelease(cert);
  if (status != errSecSuccess || !cert) {
    [self errorWithMessage:@"ErrorIdentityCopyCertificate" error:error];
    return NULL;
  }
  return certificate;
}
...
+ (LHSecKeyRef)keyWithIdentity:(LHSecIdentityRef)identity error:(NSError *__autoreleasing *)error
{
  ...
  LHSecKeyRef key = CFBridgingRelease(k);
  if (status != errSecSuccess || !k) {
    [self errorWithMessage:@"ErrorIdentityCopyPrivateKey" error:error];
    return NULL;
  }
  return key;
}
...
+ (NSError *)errorWithMessage:(NSString*)message
{
  ...
  return [NSError errorWithDomain:@"SecManager" code:400 userInfo:info];
}

+ (void)errorWithMessage:(NSString*)message error:(NSError *__autoreleasing *)error
{
  if (error) *error = [self errorWithMessage:message];
  return;
}
